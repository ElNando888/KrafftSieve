...
def primeWindow (n : ℕ) : Finset ℕ := sorry

/-- The main term of the sieve: 2H(n) -/
noncomputable def mainTerm (n : ℕ) : ℝ := sorry

/-- The total resonance capacity: Σ (2/p)·cos(π/p) -/
noncomputable def resonanceCapacity (n : ℕ) : ℝ := sorry
...
-/
noncomputable def muMin (n : ℕ) : ℝ := sorry
