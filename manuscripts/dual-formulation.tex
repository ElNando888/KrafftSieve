...
\begin{enumerate}
    ...
    \item \textbf{Riemann Sum Quadrature}: The Lebesgue integral on $[0,1]$ and the discrete sums differ by the grid spacing $dx = 1/q(n)$. Since the continuous basis cosines and their products are band-limited, we formalize exact quadratures over the full grid $\mathbb{Z}/q(n)\mathbb{Z}$:
    \begin{align}
    \int_{X_0} (\text{coeCLM}_0(n, h)(t))^2 d\mu_0 &= \frac{1}{q(n)} \sum_{x \in q(n)} (\text{evalOnGrid}(n, h)(x))^2 \\
    \int_{X_0} c_{\text{cont}_0}(n, t) (\text{coeCLM}_0(n, h)(t))^2 d\mu_0 &= \frac{1}{q(n)} \sum_{x \in q(n)} c_{\text{cont}_0}(n, \text{gridPt}(n, x)) (\text{evalOnGrid}(n, h)(x))^2
    \end{align}
...
theorem denominator_quadrature (n : $\mathbb{N}$) (h : H$_0$ n) :
    $\int$ x, ((coeCLM$_0$ n h : X$_0$ $\to$ $\mathbb{R}$) x) ^ 2 $\partial\mu_0$ =
      (1 / (q n : $\mathbb{R}$)) * $\sum$ x $\in$ Finset.range (q n), (evalOnGrid n h x) ^ 2

theorem numerator_quadrature (n : $\mathbb{N}$) (h : H$_0$ n) :
    $\int$ x, c\_cont$_0$ n x * ((coeCLM$_0$ n h : X$_0$ $\to$ $\mathbb{R}$) x) ^ 2 $\partial\mu_0$ =
      (1 / (q n : $\mathbb{R}$)) * $\sum$ x $\in$ Finset.range (q n), c\_cont$_0$ n (gridPt n x) * (evalOnGrid n h x) ^ 2
