...
\begin{enumerate}
    ...
    \item \textbf{Riemann Sum Quadrature}: The Lebesgue integral on $[0,1]$ and the discrete sums differ by the grid spacing $dx = 1/q(n)$. Since the continuous window is compactly supported and hence not band-limited, we formalize approximate quadratures with explicit error bounds $E_1(n)$ and $E_2(n)$:
    \begin{align}
    ...
    \end{align}
    These correspond to \texttt{denominator\_quadrature} and \texttt{numerator\_quadrature} in the codebase, where the error bounds satisfy $\lim_{n \to \infty} E_1(n) = 0$ and $\lim_{n \to \infty} E_2(n) = 0$.
