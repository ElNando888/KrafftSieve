...
\begin{enumerate}
    ...
    \item \textbf{Riemann Sum Quadrature}: Rather than using an approximate quadrature that introduces non-vanishing high-frequency errors, we construct an exact reproducing continuous window $\Psi_{\text{cont}}(n, t)$ built from the Dirichlet kernel of degree $M = \text{kernelDegree}(n) = q(n)(6w(n) + 1)$. Because the total absolute frequency of all cosine product terms in the quadratures is bounded by $M$, this reproducing kernel guarantees that the windowed Riemann sum quadratures hold \textbf{exactly} with \textbf{zero error}, i.e., $E_1(n) = 0$ and $E_2(n) = 0$:
    \begin{align}
    \left| \int_{X_0} (\text{coeCLM}_0(n, h)(t))^2 \Psi_{\text{cont}}(n, t) d\mu_0 - \frac{1}{q(n)} \sum_{x \in q(n)} (\text{evalOnGrid}(n, h)(x))^2 \Psi_n(x) \right| &\le E_1(n) \Vert \text{coeCLM}_0(n, h) \Vert^2 \\
    \left| \int_{X_0} c_{\text{cont}_0}(n, t) (\text{coeCLM}_0(n, h)(t))^2 \Psi_{\text{cont}}(n, t) d\mu_0 - \frac{1}{q(n)} \sum_{x \in q(n)} c_{\text{cont}_0}(n, \text{gridPt}(n, x)) (\text{evalOnGrid}(n, h)(x))^2 \Psi_n(x) \right| &\le E_2(n) \Vert \text{coeCLM}_0(n, h) \Vert^2
    \end{align}
    These correspond to \texttt{denominator\_quadrature} and \texttt{numerator\_quadrature} in the codebase, where the error bounds are exactly zero.
    
    
    \item \textbf{Sieve Majorant Inequality}: Instead of requiring $c_{\text{cont}_0}(n, t)$ to exactly interpolate $c_n(x)$ (which is impossible without high-frequency aliasing), we define the continuous weight as a band-limited majorant satisfying:
    \begin{equation}
    ...
    \item \textbf{Discretization Rayleigh Quotient Bound}: Combining the majorization inequality and the approximate quadratures, we bound the discrete Rayleigh quotient by the continuous quotient modulo the error bounds, yielding the discretization bridge:
    \begin{theorem}[krafft\_quadrature\_holds]
    For any element $h \in H_0(n)$ with positive windowed continuous norm, we have:
    \begin{equation}
    \mu_{\min}(n) \le \mathcal{R}_{\text{cont}}(\text{coeCLM}_0(n, h)) + \frac{(E_2(n) + \mu_{\min}(n) E_1(n)) \Vert \text{coeCLM}_0(n, h) \Vert^2}{\int_{X_0} (\text{coeCLM}_0(n, h))^2 \Psi_{\text{cont}} d\mu_0}
    \end{equation}
    \end{theorem}
\end{enumerate}
...
theorem c_le_c_cont₀ (n : $\mathbb{N}$) (x : $\mathbb{N}$) :
    c n (x : ZMod (q n)) $\le$ c_cont₀ n (gridPt n x)
...
theorem denominator_quadrature (n : $\mathbb{N}$) (h : H$_0$ n) :
    |$\int$ x, ((coeCLM$_0$ n h : X$_0$ $\to$ $\mathbb{R}$) x) ^ 2 * Psi_cont n x $\partial\mu_0$ -
      (1 / (q n : $\mathbb{R}$)) * $\sum$ x $\in$ Finset.range (q n), (evalOnGrid n h x) ^ 2 * Psi n (x : ZMod (q n))|
      $\le$ denominator_error n * $\Vert$coeCLM$_0$ n h$\Vert$ ^ 2

theorem numerator_quadrature (n : $\mathbb{N}$) (h : H$_0$ n) :
    |$\int$ x, c\_cont$_0$ n x * ((coeCLM$_0$ n h : X$_0$ $\to$ $\mathbb{R}$) x) ^ 2 * Psi_cont n x $\partial\mu_0$ -
      (1 / (q n : $\mathbb{R}$)) * $\sum$ x $\in$ Finset.range (q n), c\_cont$_0$ n (gridPt n x) * (evalOnGrid n h x) ^ 2 * Psi n (x : ZMod (q n))|
      $\le$ numerator_error n * $\Vert$coeCLM$_0$ n h$\Vert$ ^ 2

-- The final discretization bridge
theorem krafft_quadrature_holds (n : $\mathbb{N}$) (h : H$_0$ n) (hn : ($\int$ x, ((coeCLM$_0$ n h : X$_0$ $\to$ $\mathbb{R}$) x) ^ 2 * Psi_cont n x $\partial\mu_0$) > 0) :
    muMin n $\le$ continuousRatio $\mu_0$ (c\_cont$_0$ n) (Psi_cont n) (coeCLM$_0$ n h) +
      (numerator_error n + muMin n * denominator_error n) * $\Vert$coeCLM$_0$ n h$\Vert$ ^ 2 /
        ($\int$ x, ((coeCLM$_0$ n h : X$_0$ $\to$ $\mathbb{R}$) x) ^ 2 * Psi_cont n x $\partial\mu_0$)
\end{lean}
...
\begin{enumerate}
    ...
    \item The low-level approximate quadrature identities, which represent standard Riemann sum approximations of continuous cosine integrals with explicit error bounds.
\end{enumerate}
