...
\begin{enumerate}
    ...
    \item \textbf{Riemann Sum Quadrature}: The Lebesgue integral on $[0,1]$ and the discrete sums differ by the grid spacing $dx = 1/q(n)$. Since the continuous basis cosines and their products are band-limited, we formalize exact quadratures using the continuous window function $\Psi_{\text{cont}}$ and the discrete window $\Psi$:
    \begin{align}
    \int_{X_0} (\text{coeCLM}_0(n, h)(t))^2 \Psi_{\text{cont}}(n, t) d\mu_0 &= \frac{1}{q(n)} \sum_{x \in q(n)} (\text{evalOnGrid}(n, h)(x))^2 \Psi_n(x) \\
    \int_{X_0} c_{\text{cont}_0}(n, t) (\text{coeCLM}_0(n, h)(t))^2 \Psi_{\text{cont}}(n, t) d\mu_0 &= \frac{1}{q(n)} \sum_{x \in q(n)} c_{\text{cont}_0}(n, \text{gridPt}(n, x)) (\text{evalOnGrid}(n, h)(x))^2 \Psi_n(x)
    \end{align}
    ...
    \end{equation}
    represented in Lean by \texttt{c\_le\_c\_cont₀}.
    
    \item \textbf{Discretization Rayleigh Quotient Bound}: Combining the majorization inequality and the exact quadratures, we bound the discrete Rayleigh quotient by the continuous quotient, yielding the discretization bridge:
...
theorem denominator_quadrature (n : $\mathbb{N}$) (h : H$_0$ n) :
    $\int$ x, ((coeCLM$_0$ n h : X$_0$ $\to$ $\mathbb{R}$) x) ^ 2 * Psi_cont n x $\partial\mu_0$ =
      (1 / (q n : $\mathbb{R}$)) * $\sum$ x $\in$ Finset.range (q n), (evalOnGrid n h x) ^ 2 * Psi n ↑x
...
theorem numerator_quadrature (n : $\mathbb{N}$) (h : H$_0$ n) :
    $\int$ x, c\_cont$_0$ n x * ((coeCLM$_0$ n h : X$_0$ $\to$ $\mathbb{R}$) x) ^ 2 * Psi_cont n x $\partial\mu_0$ =
      (1 / (q n : $\mathbb{R}$)) * $\sum$ x $\in$ Finset.range (q n), c\_cont$_0$ n (gridPt n x) * (evalOnGrid n h x) ^ 2 * Psi n ↑x
