...
\author{Gemini\thanks{Google DeepMind} \and Fernando Portela\thanks{RIIS 2.0}}
\date{\today}
...
theorem mercer_theorem :
    ∃ (ι : Type*) (_ : Countable ι) 
      ...
      (σ : ι → ℝ),
      (∀ i, 0 ≤ σ i) ∧
      -- Eigenvalue equation: T_K(b_i) = σ_i b_i
...
theorem evalOnGrid_eq_spatialVector (n : ℕ) (h : H₀ n) :
    ∃ lambda : Finset (Fin (w n)) → ℝ,
      ∀ x ∈ evalInterval n, evalOnGrid n h x = spatialVector n lambda x

-- Riemann-scaled quadratures
theorem denominator_quadrature (n : ℕ) (h : H₀ n) :
    ∫ x, ((coeCLM₀ n h : X₀ → ℝ) x) ^ 2 ∂μ₀ =
      (1 / (q n : ℝ)) * ∑ x ∈ evalInterval n, (evalOnGrid n h x) ^ 2

theorem numerator_quadrature (n : ℕ) (h : H₀ n) :
    ∫ x, c_cont₀ n x * ((coeCLM₀ n h : X₀ → ℝ) x) ^ 2 ∂μ₀ =
      (1 / (q n : ℝ)) * ∑ x ∈ evalInterval n, c n (x : ZMod (q n)) * (evalOnGrid n h x) ^ 2
...
theorem continuousRatio_limit (μ : Measure X) [IsFiniteMeasure μ]
    ...
    Filter.Tendsto (fun n ↦ continuousRatio μ c_cont (f_seq n)) Filter.atTop
      (nhds (continuousRatio μ c_cont f))
