...
unconditional bounds currently available are of order $O(x^{0.525})$.
...
Our first primary formalized milestone is the \textit{Legendre targeting property}: for any
integer $x \in B_n^{(k)}$, the candidates $6x+1$ and $6x-1$ natively satisfy the property of
living strictly between consecutive squares. This combinatorial translation means that proving
Legendre's Conjecture is reduced to proving that at least one element $x$ in at least one Krafft
interval produces a prime out of $\{6x-1, 6x+1\}$.
...
connecting potential dual-strikes by the same prime $p$. Because traversing this distance requires
steps far exceeding the length of $B_n^{(k)}$, we formally derive the ``wasted cover'' lemma: a
large prime cannot simultaneously strike a candidate of the form $6x+1$ and a distinct candidate
$6y-1$ without exceeding the combinatorial boundaries. 
