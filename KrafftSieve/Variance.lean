...
-- `induction'`, `native_decide`, flexible `simp`) that would
-- require major rewrites to remove.
  ...
  haveI : NeZero (q n) := ⟨by
    exact Finset.prod_ne_zero_iff.mpr fun p hp =>
      Nat.Prime.ne_zero <|
        Finset.mem_filter.mp hp |>.2.2⟩
  (1 / (q n : ℝ)) *
    ∑ x : ZMod (q n), (f n r x - μ n r) ^ 2

/-
Lemma: The Fourier coefficient at 0 is equal to the mean
density μ.
-/
...
-/
lemma sum_exp_orthogonality (n : ℕ)
    (k : ZMod (q n)) :
    ∑ h : ZMod (q n),
    ...
    rw [ ← Finset.sum_subset
      ( Finset.subset_univ ( Finset.image (
        fun x : ℕ => ↑x : ℕ → ZMod ( q n ) )
        ( Finset.range ( q n ) ) ) ) ];
...
-/
lemma sum_orthogonality_application (n : ℕ)
    (r : Fin (w n) → ℕ) :
    ∑ x : ZMod (q n), ∑ y : ZMod (q n),
      (f n r x : ℂ) * (f n r y : ℂ) *
      ∑ h : ZMod (q n),
        Complex.exp (2 * Real.pi * Complex.I *
          h.val * (y - x).val / (q n : ℂ)) =
...
Fourier coefficient.
-/
lemma f_hat_normSq_expansion (n : ℕ)
    (r : Fin (w n) → ℕ) (h : ZMod (q n)) :
    ...
    have h_conj : starRingEnd ℂ (f_hat n r h) =
      ...
      have h_exp : ∀ x : ZMod (q n),
        ...
        norm_cast ; aesop;
      aesop;
    convert congr_arg₂ ( · * · ) h_def h_conj
      using 1 <;> ring_nf;
    · norm_num [ Complex.mul_conj,
    ...
    · -- By combining the exponents, we can see that
      ...
      have h_exp : ∀ x y : ZMod (q n),
        ...
        -- Since $y$ and $x$ are elements of
        -- $ZMod (q n)$, their values are integers
...
/-
Standard Parseval's Identity: The sum of the squared
magnitudes of the Fourier coefficients equals the average
of the squared function values.
-/
lemma parseval_standard (n : ℕ)
    (r : Fin (w n) → ℕ) :
    ∑ h : ZMod (q n),
        Complex.normSq (f_hat n r h) =
      (1 / (q n : ℝ)) *
...
-/
lemma variance_expansion (n : ℕ)
    (r : Fin (w n) → ℕ) :
    σ_sq n r =
      (1 / (q n : ℝ)) *
        ∑ x : ZMod (q n), (f n r x)^2 -
      (μ n r)^2 := by
      unfold σ_sq μ;
      rw [ show f n r =
        fun x => if x ∈ A n r then 1 else 0
...
-/
theorem parseval_identity (n : ℕ)
    (r : Fin (w n) → ℕ) :
    σ_sq n r =
      ∑ h : ZMod (q n),
        if h = 0 then 0
        else Complex.normSq (f_hat n r h) := by
...
lemma q_odd (n : ℕ) : Odd (q n) := by
  ...
  exact Nat.odd_iff.mpr
    ( Finset.prod_nat_mod _ _ _ ▸ by
      rw [ Finset.prod_eq_one fun x hx =>
        Nat.odd_iff.mp ( h_odd_primes x hx ) ]
      norm_num )
...
-/
lemma f_hat_conj_symm (n : ℕ)
    (r : Fin (w n) → ℕ) (h : ZMod (q n)) :
    f_hat n r (-h) =
      starRingEnd ℂ (f_hat n r h) := by
      unfold f_hat;
...
-/
lemma harmonic_variance_lower_bound (n : ℕ)
    (r : Fin (w n) → ℕ)
    (h : ZMod (q n)) (h_ne_zero : h ≠ 0) :
    σ_sq n r ≥
