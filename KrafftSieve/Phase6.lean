...
lemma krafft_pair_fourier (p x : ℕ) (hp : p.Prime) (hp5 : 5 ≤ p) :
  ...
  -- Let $r = \frac{p+1}{6}$.
  set r := (p + 1) / 6;
  -- Consider the sum $\sum_{k=0}^{p-1} \cos\left(\frac{2\pi k (x - r)}{p}\right) + \cos\left(\frac{2\pi k (x + r)}{p}\right)$.
  have h_sum : ∑ k ∈ Finset.range p, (Real.cos (2 * Real.pi * k * (x - r) / p) + Real.cos (2 * Real.pi * k * (x + r) / p)) = p * (if x ≡ r [MOD p] ∨ x ≡ p - r [MOD p] then 1 else 0) := by
    have h_sum : ∀ m : ℤ, (∑ k ∈ Finset.range p, Real.cos (2 * Real.pi * k * m / p)) = if m % p = 0 then p else 0 := by
      intro m; split_ifs <;> simp_all +decide [ ← ZMod.intCast_zmod_eq_zero_iff_dvd ] ;
      · rw [ ZMod.intCast_zmod_eq_zero_iff_dvd ] at *;
        obtain ⟨ k, rfl ⟩ := ‹ ( p : ℤ ) ∣ m › ; norm_num [ mul_assoc, mul_comm, mul_left_comm, div_eq_mul_inv, hp.ne_zero ] ;
        exact Eq.trans ( Finset.sum_congr rfl fun _ _ => by rw [ Real.cos_eq_one_iff ] ; use k * ‹ℕ›; push_cast; ring ) ( by norm_num );
      · -- Let $z = e^{2 \pi i m / p}$. Since $m$ is not divisible by $p$, $z$ is a primitive $p$-th root of unity.
        set z : ℂ := Complex.exp (2 * Real.pi * Complex.I * m / p)
        have hz : ∑ k ∈ Finset.range p, z ^ k = 0 := by
          rw [ geom_sum_eq ];
          · rw [ ← Complex.exp_nat_mul, mul_comm, Complex.exp_eq_one_iff.mpr ⟨ m, by ring_nf; norm_num [ hp.ne_zero ] ⟩ ] ; norm_num;
          · rw [ Ne.eq_def, Complex.exp_eq_one_iff ];
            field_simp;
            exact fun ⟨ n, hn ⟩ => ‹¬ ( m : ZMod p ) = 0› <| by rw [ div_eq_iff ( Nat.cast_ne_zero.mpr hp.ne_zero ) ] at hn; norm_cast at *; aesop;
    ...
    · split_ifs <;> simp_all +decide [ ← Int.natCast_modEq_iff ];
      · have := Int.dvd_sub ‹ ( p : ℤ ) ∣ x + r › ‹ ( p : ℤ ) ∣ x - r ›; norm_num at this;
        norm_cast at this; have := Nat.dvd_trans ( Nat.dvd_refl p ) this; simp_all +decide [ ← two_mul, Nat.Prime.dvd_mul ] ;
        exact absurd ( this.resolve_left ( Nat.not_dvd_of_pos_of_lt ( by norm_num ) ( by linarith ) ) ) ( Nat.not_dvd_of_pos_of_lt ( Nat.div_pos ( by linarith ) ( by norm_num ) ) ( Nat.div_lt_of_lt_mul <| by linarith ) );
      · cases ‹ ( x : ℤ ) ≡ r [ZMOD p] ∨ _› <;> simp_all +decide [ ← ZMod.intCast_zmod_eq_zero_iff_dvd, ← ZMod.intCast_eq_intCast_iff ];
      ...
      · simp_all +decide [ ← ZMod.intCast_zmod_eq_zero_iff_dvd, ← ZMod.intCast_eq_intCast_iff ];
        ...
        rw [ Nat.cast_sub ( show r ≤ p from Nat.div_le_of_le_mul <| by linarith ) ] at * ; aesop;
  -- Split the sum into two parts: one from $k=0$ to $k=\frac{p-1}{2}$ and one from $k=\frac{p+1}{2}$ to $k=p-1$.
  have h_split : ∑ k ∈ Finset.range p, (Real.cos (2 * Real.pi * k * (x - r) / p) + Real.cos (2 * Real.pi * k * (x + r) / p)) = 2 + 2 * ∑ k ∈ Finset.Ico 1 ((p + 1) / 2), (Real.cos (2 * Real.pi * k * (x - r) / p) + Real.cos (2 * Real.pi * k * (x + r) / p)) := by
    have h_split : ∑ k ∈ Finset.range p, (Real.cos (2 * Real.pi * k * (x - r) / p) + Real.cos (2 * Real.pi * k * (x + r) / p)) = 2 + ∑ k ∈ Finset.Ico 1 ((p + 1) / 2), (Real.cos (2 * Real.pi * k * (x - r) / p) + Real.cos (2 * Real.pi * k * (x + r) / p)) + ∑ k ∈ Finset.Ico ((p + 1) / 2) p, (Real.cos (2 * Real.pi * k * (x - r) / p) + Real.cos (2 * Real.pi * k * (x + r) / p)) := by
      rw [ Finset.sum_Ico_eq_sub _ _, Finset.sum_Ico_eq_sub _ _ ] <;> norm_num; all_goals omega;
    -- Notice that $\sum_{k=\frac{p+1}{2}}^{p-1} \cos\left(\frac{2\pi k (x - r)}{p}\right) + \cos\left(\frac{2\pi k (x + r)}{p}\right)$ is equal to $\sum_{k=1}^{\frac{p-1}{2}} \cos\left(\frac{2\pi (p - k) (x - r)}{p}\right) + \cos\left(\frac{2\pi (p - k) (x + r)}{p}\right)$.
    have h_symm : ∑ k ∈ Finset.Ico ((p + 1) / 2) p, (Real.cos (2 * Real.pi * k * (x - r) / p) + Real.cos (2 * Real.pi * k * (x + r) / p)) = ∑ k ∈ Finset.Ico 1 ((p + 1) / 2), (Real.cos (2 * Real.pi * (p - k) * (x - r) / p) + Real.cos (2 * Real.pi * (p - k) * (x + r) / p)) := by
      ...
      · intro k hk; rw [ Nat.cast_sub ( by linarith [ Finset.mem_Ico.mp hk ] ) ] ; ring;
    -- Notice that $\cos\left(\frac{2\pi (p - k) (x - r)}{p}\right) + \cos\left(\frac{2\pi (p - k) (x + r)}{p}\right) = \cos\left(\frac{2\pi k (x - r)}{p}\right) + \cos\left(\frac{2\pi k (x + r)}{p}\right)$.
    have h_cos_symm : ∀ k ∈ Finset.Ico 1 ((p + 1) / 2), Real.cos (2 * Real.pi * (p - k) * (x - r) / p) + Real.cos (2 * Real.pi * (p - k) * (x + r) / p) = Real.cos (2 * Real.pi * k * (x - r) / p) + Real.cos (2 * Real.pi * k * (x + r) / p) := by
      intro k hk; ring_nf ;
      norm_num [ mul_assoc, mul_comm Real.pi _, hp.ne_zero ];
      norm_num [ mul_assoc, mul_left_comm, hp.ne_zero ];
      norm_num [ Real.cos_add, Real.sin_add, Real.cos_sub, Real.sin_sub ] ; ring;
      norm_num [ mul_two, Real.sin_add, Real.cos_add ];
      norm_num [ show Real.sin ( Real.pi * r ) = 0 from Real.sin_eq_zero_iff.mpr ⟨ r, by push_cast; ring ⟩ ];
    rw [ h_split, h_symm, Finset.sum_congr rfl h_cos_symm ] ; ring;
  convert congr_arg ( fun y : ℝ => y / p ) h_sum.symm using 1;
  · split_ifs <;> simp_all +decide [ ← ZMod.natCast_eq_natCast_iff, Nat.cast_sub ( show r ≤ p from Nat.div_le_of_le_mul <| by linarith ) ];
...
lemma cosine_Icc_dirichlet_bound (A B : ℕ) (θ : ℝ)
  ...
  · -- Use the formula for the sum of a geometric series.
    have h_geo_series : ∑ x ∈ Finset.Icc A B, Complex.exp (2 * Real.pi * θ * x * Complex.I) = Complex.exp (2 * Real.pi * θ * A * Complex.I) * (1 - Complex.exp (2 * Real.pi * θ * (B - A + 1) * Complex.I)) / (1 - Complex.exp (2 * Real.pi * θ * Complex.I)) := by
      rw [ eq_div_iff ];
      · erw [ Finset.sum_Ico_eq_sum_range ];
        convert Finset.sum_range_sub' ( fun x => Complex.exp ( 2 * Real.pi * θ * ( A + x ) * Complex.I ) ) ( B + 1 - A ) using 1 <;> norm_num [ Complex.exp_add, mul_add, add_mul, Finset.sum_range_succ ] ; ring;
        · simp +decide only [mul_comm, mul_assoc, Finset.mul_sum _ _ _, mul_left_comm];
        · rw [ Nat.cast_sub ( by linarith ) ] ; push_cast ; rw [ ← Complex.exp_add ] ; ring;
      · norm_num [ Complex.ext_iff, Complex.exp_re, Complex.exp_im ];
        contrapose! h_not_int;
        rw [ sub_eq_zero, eq_comm, Real.cos_eq_one_iff ] at h_not_int; obtain ⟨ k, hk ⟩ := h_not_int.1; exact ⟨ k, by nlinarith [ Real.pi_pos ] ⟩ ;
    -- Use the fact that $|1 - e^{i\phi}| = 2|\sin(\phi/2)|$ to simplify the expression.
    have h_abs : Complex.normSq (1 - Complex.exp (2 * Real.pi * θ * (B - A + 1) * Complex.I)) ≤ 4 ∧ Complex.normSq (1 - Complex.exp (2 * Real.pi * θ * Complex.I)) = 4 * Real.sin (Real.pi * θ) ^ 2 := by
      norm_num [ Complex.normSq, Complex.exp_re, Complex.exp_im ];
      exact ⟨ by nlinarith only [ Real.cos_sq' ( 2 * Real.pi * θ * ( B - A + 1 ) ) ], by rw [ show 2 * Real.pi * θ = 2 * ( Real.pi * θ ) by ring, Real.sin_two_mul, Real.cos_two_mul ] ; nlinarith only [ Real.sin_sq_add_cos_sq ( Real.pi * θ ) ] ⟩;
    -- Use the fact that $|e^{i\phi}| = 1$ to simplify the expression.
    have h_abs_exp : Complex.normSq (∑ x ∈ Finset.Icc A B, Complex.exp (2 * Real.pi * θ * x * Complex.I)) ≤ 4 / (4 * Real.sin (Real.pi * θ) ^ 2) := by
      rw [ h_geo_series, Complex.normSq_div ];
      rw [ Complex.normSq_mul, Complex.normSq_eq_norm_sq, Complex.norm_exp ] ; norm_num [ h_abs ];
      gcongr ; aesop;
