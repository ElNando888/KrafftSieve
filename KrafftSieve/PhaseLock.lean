...
def PhaseLockedBasis (n d : ℕ) (basis : Fin d → Finset (Fin (w n))) : Prop :=
  ∀ I : Finset (Fin d), I.Nonempty →
    IsPhaseLocked n (I.fold symmDiff ∅ basis)

/-- The Phase-Locked Subspace Theorem:
If there exists a Phase-Locked Basis of dimension d, then the subspace clique
generated by this basis is a valid negative clique in the Ridge Graph. -/
theorem phase_locked_clique_penalty_neg (n d : ℕ) (basis : Fin d → Finset (Fin (w n)))
...
theorem exists_large_clique_of_neg_penalty (n : ℕ) (hn : 1000 ≤ n) :
  ...
  have h_clique := phase_locked_clique_penalty_neg n d basis h_lock
  use subspaceClique n d basis
  refine ⟨ ?_, h_clique, ?_ ⟩
  · sorry -- Prove IsClique from h_clique
