...
/-- A linear subspace of F_2^{w(n)} generated by d basis vectors,
represented as a set of subsets. -/
def subspaceClique (n : ℕ) (d : ℕ) (basis : Fin d → Finset (Fin (w n))) :
    Finset (Finset (Fin (w n))) :=
  (Finset.univ : Finset (Finset (Fin d))).image (fun I => I.fold symmDiff ∅ (fun i => basis i))

/-- The product of primes associated with a given basis vector (or subset of primes). -/
...
generated by this basis has non-positive off-diagonal penalties. -/
theorem phase_locked_subspace_penalty_neg (n d : ℕ) (basis : Fin d → Finset (Fin (w n)))
...
lemma exists_phase_locked_basis (n d : ℕ) (hn : 1000 ≤ n)
    (hd : 2 ^ d > 1 + 2 * (∑ x ∈ evalInterval n, c n (x : ZMod (q n))) / (evalInterval n).card) :
    ∃ basis : Fin d → Finset (Fin (w n)), PhaseLockedBasis n d basis := by
  sorry

/--
Phase 6 Main Theorem: For sufficiently large n, there exists a subspace of subsets whose
off-diagonal penalties are non-positive, and whose size satisfies the density bound.
-/
theorem exists_large_subspace_of_neg_penalty (n : ℕ) (hn : 1000 ≤ n) :
    ∃ C : Finset (Finset (Fin (w n))),
      (∀ S ∈ C, ∀ T ∈ C, S ≠ T → penaltyMatrixEntry n S T ≤ 0) ∧
      (C.card : ℝ) >
        1 + 2 * (∑ x ∈ evalInterval n, c n (x : ZMod (q n))) / (evalInterval n).card := by
  -- We use the existence of a negative subspace clique to satisfy the theorem.
  obtain ⟨d, hd⟩ : ∃ d : ℕ, 2 ^ d > 1 +
      2 * (∑ x ∈ evalInterval n, c n (x : ZMod (q n))) / (evalInterval n).card := by
    sorry
  -- We use the combinatorial argument that a phase-locked subgroup exists.
  obtain ⟨basis, h_lock⟩ := exists_phase_locked_basis n d hn hd
  have h_penalty := phase_locked_subspace_penalty_neg n d basis h_lock
  use subspaceClique n d basis
  refine ⟨ h_penalty, ?_ ⟩
  · sorry -- Prove cardinality is 2^d
