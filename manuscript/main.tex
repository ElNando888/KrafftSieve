...
rigorously establishing that multidimensional cross-harmonic interactions are analytically necessary to produce a favorable sieve density. We conclude
...
Crucially, our formalization also provides a profound unconditional result regarding the limits of
...
Because $g_i(x)$ is evaluated globally over $\mathbb{Z}/q\mathbb{Z}$ but operates strictly on the
local modular topology of its corresponding prime $\mathbb{Z}/p_i\mathbb{Z}$, its transform
unavoidably manifests a sparse \textit{Dirac Comb} structure. Lean 4 rigorously verifies this
...
\textbf{Resonant Sieve Equation} (\href{\urlpfx/KrafftSieve/ThirdHarmonic.lean#L614}{\texttt{resonant\_sieve\_equation}}):
$$ S_2(n, W) = S_1(n, W) \sum_{i=1}^{w(n)} \frac{2}{p_i} + q \sum_{i=1}^{w(n)} \sum_{k \neq 0} \hat{W}\!\left(k \frac{q}{p_i}\right) \frac{2}{p_i} \cos\!\left( \frac{2 \pi k r^K_i}{p_i} \right) $$
...
primorial parity split ($p_i \equiv 1 \pmod{6}$ versus $p_i \equiv 5 \pmod{6}$) determines the
...
$\sum \frac{2}{p_i} \cos(\pi/p_i) < \sum \frac{2}{p_i}$ strictly holds. However, the profound
structural consequence is not the simple inequality itself, but the spectral signature of the
deficit: the main domain volume notoriously diverges while the isolated resonance capacity is forced to
strictly converge. Formalizing this fundamental separation within Lean~4
...
\le \sum_{p_i \in \mathcal{P}_n} \frac{2}{p_i} \left(\frac{1}{2}\left(\frac{\pi}{p_i}\right)^2\right) = \sum_{p_i \in \mathcal{P}_n} \frac{\pi^2}{p_i^3} $$
...
inclusion-exclusion principle. The "perfect" spatial weight function $\omega(x)$, which evaluates exactly to
...
Gemini 3.1 Pro was instrumental in clarifying the theoretical ideas and generating precise prompt heuristics for the formal prover, while Aristotle served as the primary mechanical workhorse securing the dense derivations.
...
The following directed graphs map the logical dependencies between the central formalized results. To maintain structural clarity and avoid overwhelming edge density, basic definitions and foundational types are generally excluded from the hierarchy. 
...
\begin{itemize}
    ...
    \item \textbf{Directed arrows} ($A \rightarrow B$): Logical dependence, signifying that proof $A$ explicitly ``uses'' or ``applies'' result $B$ to close a goal.
\end{itemize}
