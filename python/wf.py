...
def resonator(x_range, primes, alpha, nconv=2, harmonic=3.0):
    # Cosine Resonator
    ...
    return np.abs((1.0 - alpha * penalty_sum))**nconv
...
def weight_vectorized(n, x_range, lower, upper, primes, alpha, nconv=2):
    ...
    return bump * resonator(x_range, primes, alpha, nconv)
...
def evaluate_alpha(alpha_step, n, x_range, lower, upper, primes, nconv, c_vals):
    ...
    if s1 > 0:
        ...
        if s2 < s1:
            return (s2 / s1, test_alpha, w_vals, resonator(x_range, primes, test_alpha, nconv), s1, s2)

        return (s2 / s1, test_alpha, w_vals, None, s1, s2)

    return (float('inf'), test_alpha, None, None, 0, 0)
...
def run_test(nconv, results_store):
    ...
    bottom = math.floor(math.exp(((nconv-1) + 2.65) / 2.05)) + 1
    # alpha_steps = list(range(115, 441, 2))
    # alpha_steps = list(range(33, 524, 3))
    alpha_steps = list(math.exp(3-x/20) for x in range(1,121,1))

    for n in range(bottom, limit):
        if n in results_store:
            v = results_store[n][0]
            print(f"{n:<3} | {'-':<10} | {'-':<14} | {'-':<14} | {'-':<10} | True ({v[0]}, {v[3]:<6.3f})", flush=True)
            continue        
        ...
        with ProcessPoolExecutor() as executor:
            ...
            for future in futures:
                ratio, test_alpha, w_vals, r_vals, s1, s2 = future.result()
                if ratio < best_ratio:
                    best_ratio = ratio
        ...
        if success:
            if n not in results_store:
                results_store[n] = []
            results_store[n].append((nconv, best_weights, best_reson, best_alpha))
...
if __name__ == '__main__':
    ...
    for n in sorted(results_store.keys()):
        print(f"\n[n = {n}]")
        for nconv, weights, reson, alpha in results_store[n]:
