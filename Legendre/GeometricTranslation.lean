...
PROVIDED SOLUTION
If a ≡ b (mod p) and both a, b are in Bk n k (an interval of length ≤ 2n), then |a - b| < 2n+1 ≤ p 
(since p > 2n). But a ≡ b (mod p) means p ∣ (a - b). Since |a - b| < p, we must have a - b = 0, so 
a = b.
-/
...
PROVIDED SOLUTION
By definition, sieve_hit_plus p m alpha = (alpha * m : ℤ) and sieve_hit_minus p m alpha = 
(-alpha * m : ℤ). Their difference in ZMod p is (alpha * m - (-alpha * m) : ℤ) = 
(2 * alpha * m : ℤ). 
This is a direct computation in ZMod p using the definitions.
-/
theorem krafft_translation_vector (p : ℕ) (m : ℤ) (alpha : ℤ)
    (_halpha : alpha = 1 ∨ alpha = -1) (_hp_eq : (p : ℤ) = 6 * m + alpha) :
...
PROVIDED SOLUTION
From p = 6m + α with α ∈ {1,-1}, we get m = (p - α)/6. So |m| = (p - α)/6 (m is positive since 
p ≥ 5). Then 2|m| = (p - α)/3 ≥ (p - 1)/3. Cases on alpha: if alpha=1, m = (p-1)/6, 2m = (p-1)/3 ≥ 
(p-1)/3. If alpha=-1, m = (p+1)/6, 2m = (p+1)/3 ≥ (p-1)/3.
-/
theorem krafft_parameter_bound (p : ℕ) (m : ℤ) (alpha : ℤ)
    (halpha : alpha = 1 ∨ alpha = -1) (hp_eq : (p : ℤ) = 6 * m + alpha)
    (hp5 : 5 ≤ p) :
    2 * |m| ≥ ((p : ℤ) - 1) / 3 := by
      rcases halpha with halpha | halpha <;> cases abs_cases m <;> omega
...
PROVIDED SOLUTION
From p ∣ 6*x_plus+1 and p ∣ 6*x_minus-1, we get p ∣ (6*x_plus+1) - (6*x_minus-1) = 
6*(x_plus - x_minus) + 2. Also p ∣ 6*(x_plus - x_minus) + 2. Since p = 6m+α, we need 
x_plus - x_minus ≡ 2αm (mod p). From p ∣ 6*x_plus+1, we get 6*x_plus ≡ -1 (mod p). From 
p ∣ 6*x_minus-1, we get 6*x_minus ≡ 1 (mod p). So 6*(x_plus - x_minus) ≡ -2 (mod p). 
Since p = 6m+α, 6 has an inverse mod p (as p ≥ 5 is prime, gcd(6,p)=1). We have 
x_plus - x_minus ≡ -2/6 (mod p). But 6*αm ≡ -α² = -1 (mod p), so 1/6 ≡ -αm (mod p), 
and -2/6 ≡ 2αm (mod p).
-/
theorem paired_strike_separation (n : ℕ) (k : Fin 6) (p : ℕ) (_hp : p ∈ P_large n)
    (m : ℤ) (alpha : ℤ) (halpha : alpha = 1 ∨ alpha = -1)
    (hp_eq : (p : ℤ) = 6 * m + alpha) (_hp_prime : Nat.Prime p)
    (x_plus x_minus : ℤ) (_hxp : x_plus ∈ Bk n k) (_hxm : x_minus ∈ Bk n k)
    (h_hit_plus : (p : ℤ) ∣ 6 * x_plus + 1) (h_hit_minus : (p : ℤ) ∣ 6 * x_minus - 1) :
    x_plus - x_minus ≡ 2 * alpha * m [ZMOD (p : ℤ)] := by
      -- From the divisibility conditions, we have $6x_plus ≡ -1 \pmod{p}$ and 
      -- $6x_minus ≡ 1 \pmod{p}$.
      ...
      · obtain ⟨ k, hk ⟩ := h_mod_diff;
        exact ⟨ k * m + ( x_plus - x_minus ), 
          by cases le_or_gt 0 m <;> nlinarith ⟩
